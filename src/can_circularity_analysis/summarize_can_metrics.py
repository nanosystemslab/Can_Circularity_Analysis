...
def overlay_circles(cases, out_png, legend=False, legend_limit=10, legend_by_parent=False):
    ...
    ax.set_xlabel("x / r_fit"); ax.set_ylabel("y / r_fit")
    if legend:
        ax.legend(loc="center left", bbox_to_anchor=(1.02, 0.5),
                  fontsize="x-small", ncol=1, frameon=False, borderaxespad=0.0)
    plt.savefig(out_png, bbox_inches='tight', dpi=150); plt.close()

def overlay_rims(cases, out_png, max_points_per=400, legend=False, legend_limit=10, legend_by_parent=False):
    plt.figure()
    ax = plt.gca()
    for i, c in enumerate(cases):
...
def overlay_ellipses(cases, out_png, legend=False, legend_limit=10, legend_by_parent=False):
    ...
    for i, c in enumerate(cases):
        ...
        if not e:
            continue
        xr, yr = ellipse_pts(e["MA_px"]/c["radius_px"], e["ma_px"]/c["radius_px"], e["angle_deg"])
        label = None
        if legend and i < legend_limit:
            label = c["parent"] if legend_by_parent else c["stem"]
...
def polar_deviation(cases, out_png, use_mm=False, show_mean_band=True, legend=False, legend_limit=10, legend_by_parent=False):
    plt.figure()
    ax = plt.gca()
    all_theta, all_dev = [], []
    for i, c in enumerate(cases):
        ...
        all_theta.append(th); all_dev.append(dev)
    ...
    if show_mean_band:
        ...
        curves = []
        for th, dv in zip(all_theta, all_dev):
            thd = np.degrees(th)
            order = np.argsort(thd)
            thd = thd[order]; dvd = dv[order]
            curves.append(np.interp(th_grid, thd, dvd, left=np.nan, right=np.nan))
        arr = np.array(curves)
        mean = np.nanmean(arr, axis=0)
        std  = np.nanstd(arr, axis=0)
        ax.plot(th_grid, mean, linewidth=2)
        ax.fill_between(th_grid, mean-std, mean+std, alpha=0.2, label="mean ± std" if legend else None)
...
def histograms(cases, out_dir):
    ...
    if len(diam_mm) > 0:
        plt.figure(); plt.hist(diam_mm, bins=20)
        plt.xlabel("Diameter (mm)"); plt.ylabel("Count"); plt.title("Diameter distribution")
        plt.savefig(out_dir / "hist_diameter_mm.png", bbox_inches='tight', dpi=150); plt.close()

    if len(rms_mm) > 0:
        plt.figure(); plt.hist(rms_mm, bins=20)
        plt.xlabel("RMS out-of-round (mm)"); plt.ylabel("Count")
        plt.title("RMS out-of-round")
        plt.savefig(out_dir / "hist_rms_mm.png", bbox_inches='tight', dpi=150); plt.close()

    if len(std_mm) > 0:
        plt.figure(); plt.hist(std_mm, bins=20)
        plt.xlabel("STD out-of-round (mm)"); plt.ylabel("Count")
        plt.title("STD out-of-round")
        plt.savefig(out_dir / "hist_std_mm.png", bbox_inches='tight', dpi=150); plt.close()

    if len(rng_mm) > 0:
        plt.figure(); plt.hist(rng_mm, bins=20)
        plt.xlabel("RANGE out-of-round (mm)"); plt.ylabel("Count")
        plt.title("RANGE out-of-round")
        plt.savefig(out_dir / "hist_range_mm.png", bbox_inches='tight', dpi=150); plt.close()
...
def scatter_ellipse_axes(cases, out_png, legend=False, legend_limit=10, legend_by_parent=False):
    ...
    for c in cases:
        ...
        if ("MA_mm" in e) and ("ma_mm" in e):
            xs.append(e["ma_mm"]); ys.append(e["MA_mm"])
            labels.append(c["parent"] if legend_by_parent else c["stem"])
        else:
            if ("MA_px" in e) and ("ma_px" in e):
                xs.append(e["ma_px"]); ys.append(e["MA_px"])
                labels.append(c["parent"] if legend_by_parent else c["stem"])
    if len(xs) == 0:
        plt.figure(); plt.text(0.5,0.5,"No ellipse data", ha='center')
        plt.savefig(out_png, bbox_inches='tight', dpi=150); plt.close(); return
    lo = min(min(xs), min(ys)); hi = max(max(xs), max(ys))
    plt.figure()
    plt.plot(xs, ys, '.', markersize=4, label=None)
    plt.plot([lo, hi], [lo, hi])
    plt.xlabel("Minor axis"); plt.ylabel("Major axis")
...
def main():
    ...
    # plotting
    ap.add_argument("--make-plots", action="store_true", help="Also generate plots into <out-dir>/plots")
    ap.add_argument("--polar-mm", action="store_true", help="Polar Δr(θ) in mm (requires scale in JSONs)")
    ap.add_argument("--max-points-per", type=int, default=400, help="Max points per rim in overlays")
    # legends
    ap.add_argument("--legend", action="store_true", help="Add legends to overlay plots (may clutter).")
    ap.add_argument("--legend-limit", type=int, default=10, help="Max labels/entries to show in legends.")
    ap.add_argument("--legend-by-parent", action="store_true",
    ...
    if args.make_plots:
        ...
        if not cases:
            print("[warn] no cases to plot"); return

        # NEW: overlay of best-fit circles (normalized)
        overlay_circles(
            cases, plots_dir / "overlay_circles.png",
            legend=args.legend, legend_limit=args.legend_limit,
            legend_by_parent=args.legend_by_parent
        )

        # overlays (rims + ellipses)
        overlay_rims(
            cases, plots_dir / "overlay_rims.png",
            max_points_per=args.max_points_per,
            legend=args.legend, legend_limit=args.legend_limit,
            legend_by_parent=args.legend_by_parent
        )
        overlay_ellipses(
            cases, plots_dir / "overlay_ellipses.png",
            legend=args.legend, legend_limit=args.legend_limit,
            legend_by_parent=args.legend_by_parent
        )
        ...
        # polar Δr(θ)
        polar_deviation(
            cases, plots_dir / ("polar_deviation_mm.png" if args.polar_mm else "polar_deviation_px.png"),
            use_mm=args.polar_mm, show_mean_band=True,
            legend=args.legend, legend_limit=args.legend_limit,
            legend_by_parent=args.legend_by_parent
        )
        ...
        scatter_ellipse_axes(
            cases, plots_dir / "scatter_ellipse_axes.png",
            legend=args.legend, legend_limit=args.legend_limit,
            legend_by_parent=args.legend_by_parent
        )
