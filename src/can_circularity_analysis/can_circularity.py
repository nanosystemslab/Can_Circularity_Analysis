...
def calibrate_ppmm_by_click(img, known_mm=50.0, win="Calibration"):
    ...
    print(
        f"[calibrate] Click two points on the ruler that are exactly {known_mm} mm apart. Press ESC to cancel."
    )
    while True:
...
def save_scale_json(
    path, pixels_per_mm, known_mm=None, source_image=None, scale_points_full_px=None
):
...
def fit_best_circle(contour):
    ...
    model_robust, inliers = ransac(
        pts, CircleModel, min_samples=3, residual_threshold=2.0, max_trials=1000
    )
...
def pick_contour_with_filter(
    mask_or_edges, prefer_center, center_radius_frac, mm_per_px, min_diam_mm, max_diam_mm
):
    ...
    scored = sorted(
        cnts, key=lambda c: _score_contour(c, w, h, prefer_center, center_radius_frac), reverse=True
    )
...
def analyze_image(path, args):
    ...
    if args.edge_method == "binary":
        ...
        try:
            ...
            if edges is None:
                edges = cv2.Canny(
                    gray_eq,
                    max(10, args.canny_low // 2),
                    max(40, args.canny_high // 2),
                    L2gradient=True,
                )
    ...
    if diameter_mm is not None:
        if not (args.min_diameter_mm <= diameter_mm <= args.max_diameter_mm):
            raise RuntimeError(
                f"Detected rim {diameter_mm:.2f} mm is outside "
    ...
    if not args.no_overlay:
        ...
        cv2.putText(
            dbg, label, (12, 28), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 255, 255), 2, cv2.LINE_AA
        )
    ...
    metrics = {
        ...
        "pixels_per_mm": ppmm,
        "known_mm_for_scale": args.known_mm
        if (args.click_scale or args.scale_pts)
        else (scale_meta or {}).get("known_mm")
        if args.scale_from
        else None,
...
def build_argparser():
    ...
    p.add_argument(
        "--known-mm", type=float, default=50.0, help="Distance in mm for the 2 clicked points."
    )
    p.add_argument(
        "--scale-pts", type=str, default=None, help="Non-GUI scale 'x1,y1,x2,y2' (full image)."
    )
    p.add_argument(
        "--save-scale", type=str, default=None, help="Write JSON with {'pixels_per_mm': ...}."
    )
    p.add_argument(
        "--scale-from", type=str, default=None, help="Load JSON scale (reused across images)."
    )

    # Diameter filter (mm)
    p.add_argument(
        "--min-diameter-mm", type=float, default=0.0, help="Ignore rims smaller than this (mm)."
    )
    p.add_argument(
        "--max-diameter-mm", type=float, default=1e9, help="Ignore rims larger than this (mm)."
    )

    # Segmentation
    p.add_argument(
        "--edge-method", choices=["binary", "canny"], default="binary", help="Rim detection method."
    )
    p.add_argument(
        "--binary-block-size", type=int, default=51, help="Adaptive threshold block size (odd)."
    )
    ...
    p.add_argument(
        "--prefer-center", action="store_true", help="Prefer contours near image center."
    )
    p.add_argument(
        "--center-radius-frac", type=float, default=0.4, help="Center preference radius fraction."
    )
