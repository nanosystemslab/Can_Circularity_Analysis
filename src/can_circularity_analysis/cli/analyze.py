...
from can_circularity_analysis.core.calibration import parse_crop_string, save_calibration
from can_circularity_analysis.core.detection import analyze_image_file
from can_circularity_analysis.utils.file_io import (
...
Examples:
  ...
  can-analyze image2.jpg image3.jpg --scale-from calibration.json
    ...
    calib_group.add_argument(
        "--scale-pts", help="Non-GUI calibration points as 'x1,y1,x2,y2' in full image coordinates"
    )
    calib_group.add_argument("--save-scale", help="Save calibration to JSON file for reuse")
    calib_group.add_argument("--scale-from", help="Load calibration from existing JSON file")
    ...
    process_group.add_argument("--crop", help="Crop region as 'x,y,width,height' before analysis")
...
def validate_arguments(args) -> None:
    ...
    if missing_images:
        print("Error: The following image files were not found or are invalid:")
...
def analyze_single_image(image_path: str, args) -> dict:
    ...
    try:
        ...
        error_result = {"image_path": image_path, "error": str(e), "success": False}
        if not args.quiet:
