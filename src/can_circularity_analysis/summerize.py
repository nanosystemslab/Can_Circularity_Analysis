...
from ..utils.file_io import create_output_directory, find_metrics_files, load_metrics_json
...
def _create_histograms(cases: list[dict], plots_dir: Path) -> None:
    ...
    metrics = {
        ...
        "rms_out_of_round_mm": [
            c.get("rms_out_of_round_mm") for c in cases if c.get("rms_out_of_round_mm")
        ],
        "circularity": [
            c.get("circularity_4piA_P2") for c in cases if c.get("circularity_4piA_P2")
        ],
    }
...
def _create_polar_plot(cases: list[dict], plots_dir: Path, args) -> None:
    ...
    for i, case in enumerate(cases[: min(5, len(cases))]):  # Limit to first 5 for visibility
        ...
        deviation_sim = 0.1 * np.sin(3 * np.deg2rad(theta_sim)) + 0.05 * np.random.randn(
            len(theta_sim)
        )
        ...
        if successful > 0:
            ...
            if available_cols:
                ...
                for col in available_cols:
                    ...
                    if len(valid_data) > 0:
                        print(
                            f"  {col}: {valid_data.mean():.3f} ± {valid_data.std():.3f} "
