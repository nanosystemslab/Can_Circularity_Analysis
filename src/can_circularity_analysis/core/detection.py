...
from .calibration import load_calibration, calibrate_from_points, calibrate_pixels_per_mm
from .metrics import calculate_circularity_metrics, calculate_deviation_metrics, validate_diameter_range
...
def fit_circle_to_contour(contour: np.ndarray) -> Tuple[Tuple[float, float, float], np.ndarray, np.ndarray]:
...
def fit_ellipse_to_contour(contour: np.ndarray) -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            ...
            "ovalization": (major_axis - minor_axis) / ((major_axis + minor_axis)/2.0) if (major_axis + minor_axis) > 0 else None
        }
    ...
    **kwargs
    ...
    if method == "binary":
        contour, circle_params, points, inliers = select_best_contour(
            binary_mask, **{k: v for k, v in kwargs.items() if k in [
                'prefer_center', 'center_radius_fraction', 'mm_per_pixel', 
        ...
        try:
            contour, circle_params, points, inliers = select_best_contour(
                edges, **{k: v for k, v in kwargs.items() if k in [
                    'prefer_center', 'center_radius_fraction', 'mm_per_pixel',
                    'min_diameter_mm', 'max_diameter_mm'
                ]}
            )
...
def analyze_image_file(
    ...
    **kwargs
    ...
    detection_result = detect_circular_contour(
        image, 
    ...
    if mm_per_pixel:
        results.update({
            "center_mm": [(center_x + offset[0]) * mm_per_pixel, (center_y + offset[1]) * mm_per_pixel],
    ...
    if not kwargs.get('no_overlay', False):
        ...
        overlay_path = create_analysis_overlay(
            image, contour, center_x, center_y, radius, points, 
            ellipse_result, scale_metadata, offset, output_dir, stem, **kwargs
        )
...
def _setup_calibration(image_path: str, **kwargs) -> Tuple[Optional[float], Optional[dict]]:
    ...
    if kwargs.get('scale_from'):
        ...
        try:
            ...
            pixels_per_mm, scale_points = calibrate_pixels_per_mm(
                full_image.copy(), 
                known_mm=kwargs.get('known_mm', 50.0)
            )
            scale_metadata = {"scale_points_full_px": scale_points}
